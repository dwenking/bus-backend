...
public class LineController {
    ...
    public JSONResult<?> findShortestPathById(
        ...
        if (stations == null || stations.size() == 0) {
            return JSONResult.error(JSONResult.NO_DATA_ERROR, "不存在最短路径");
        }
        return JSONResult.success(stations);
    }

    @ApiOperation(value = "findShortestPathByName根据name求出两站之间的最短路径路线", notes = "5.2使用name查询某两个站台之间的最短路径")
    ...
    @ApiOperation(value = "findShortestMinTimePathByName根据name求出两站之间的最短路径中的最少时间路线", notes = "5.3如果有更多细节考虑(最少换乘)")
    @GetMapping(path = "/get/shortest/minTime/path/name/{name1}/{name2}")
    public JSONResult<?> findShortestMinTimePathByName(
            @PathVariable String name1,
            @PathVariable String name2
    ) {
    ...
    @ApiOperation(value = "findShortestMinTransferPathByName根据name求出两站之间的最短路径中的最少换乘路线", notes = "5.3如果有更多细节考虑(最少换乘)")
    @GetMapping(path = "/get/shortest/minTransfer/path/name/{name1}/{name2}")
    public JSONResult<?> findShortestMinTransferPathByName(
            @PathVariable String name1,
            @PathVariable String name2
    ) {
        ...
        for(StationPath stationPath:stationpaths){
            ...
            for(int i=0;i<stationPath.getStationList().size();i++){
                if(i!=stationPath.getStationList().size()-1){
                    ...
                    tmp+=stationPath.getStationRelationships().get(i).getName()+"->";
                }
                else {
                    tmp+=stationPath.getStationList().get(i).getName();
                }
